...
import { DocumentAreaSelect } from './DocumentAreaSelect';
...
import type { EducationDocument, EducationDocumentAssignment, WorkshopArea } from '@platform/shared';
...
export default async function EducationDocumentsPage() {
  ...
  if (isStaff) {
    ...
    try {
      ...
      console.error('[education/documents] failed to load areas', { tenant: user.tenant, error });
    }
  }
  ...
  // ── Gruppera dokument per område (samma princip som workshops-översikten) ──
  const documentsByArea = new Map<string, EducationDocument[]>();
  const documentsWithoutArea: EducationDocument[] = [];
  for (const doc of documents) {
    const areaId = typeof doc.area === 'string' ? doc.area : '';
    if (areaId) {
      if (!documentsByArea.has(areaId)) documentsByArea.set(areaId, []);
      documentsByArea.get(areaId)?.push(doc);
    } else {
      documentsWithoutArea.push(doc);
    }
  }
    ...
    <PageShell title="Utbildning" tabs={EDUCATION_TABS}>
      <div className="grid gap-6 py-6 lg:grid-cols-[340px_1fr]">
        <div className="space-y-4">
          <DocumentUploadForm areas={areas} />
        </div>
            ...
            documentGroups.map((group) => (
              <section key={group.id} className="space-y-4">
                ...
                {group.documents.map((doc) => {
                  ...
                  const currentAreaId = typeof doc.area === 'string' ? doc.area : '';
                  return (
                    <div
                      key={doc.id}
                      className="rounded-2xl border border-default bg-surface p-5 shadow-sm shadow-movexum-svart/5"
