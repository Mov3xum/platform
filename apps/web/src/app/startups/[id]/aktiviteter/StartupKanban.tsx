...
// Bolagskanbanen (CLAUDE.md § 15.7) — sex kolumner i Miro-stil. Samma
// HTML5-drag-and-drop + useOptimistic-mönster som OverviewBoard.
...
export function StartupKanban({
  ...
  function move(task: StartupBoardTask, status: StartupBoardStatus) {
    ...
    startTransition(async () => {
      ...
      const res = await moveStartupBoardTaskAction(task.id, status);
  ...
  function assign(task: StartupBoardTask, next: BoardAssignee[]) {
    startTransition(async () => {
      ...
      const res = await setTaskAssigneesAction(
        task.id,
        next.map((a) => a.id)
      );
        ...
        {STARTUP_BOARD_COLUMNS.map((col) => {
          ...
          return (
            ...
            >
              ...
              {canManage && (
                <QuickAdd startupId={startupId} status={col.id} onError={setError} />
              )}
            </div>
          );
        })}
...
function QuickAdd({
  ...
  function submit() {
    ...
    startTransition(async () => {
      const res = await createStartupBoardTaskAction({
        startupId,
        description,
        status
      });
